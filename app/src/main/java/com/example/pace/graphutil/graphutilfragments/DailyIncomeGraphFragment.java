...
public class DailyIncomeGraphFragment extends Fragment {
    public DailyIncomeGraphFragment() {
    }

    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_daily_income_graph, container, false);
    ...
    private void initViews(View view) {
        this.lineChart = view.findViewById(R.id.daily_income_lineGraph);

        this.incomeValue = view.findViewById(R.id.daily_income_graph_value);
        this.incomeValue.setText("$ "+ListHolder.getInstance().outputDailyDataList.get(
    ...
    private void addEntries() {
        this.incomeEntries = new ArrayList<>();
        ...
        for (int i = 0; i < ListHolder.getInstance().outputDailyDataList.size(); ++i) {
            if (i == ListHolder.getInstance().outputDailyDataListIndex) {
                Entry entry = new Entry(
                        i, ListHolder.getInstance().outputDailyDataList.get(i).getAverageIncome()
                );
                this.incomeEntries.add(entry);
    ...
    public void addLineData() {
        ...
        for(int i = 0; i < this.incomeEntries.size(); ++i) {
            LineDataSet dsi = new LineDataSet(Collections.singletonList(this.incomeEntries.get(i)), null);
        ...
        LineDataSet dataSet = new LineDataSet(this.incomeEntries, null);
    ...
    private float getMaximum() {
        float retVal = 0.0f;
        for (int i = 0; i < ListHolder.getInstance().outputDailyDataList.size(); ++i) {
            if (retVal < ListHolder.getInstance().outputDailyDataList.get(i).getAverageIncome()) {
                retVal = ListHolder.getInstance().outputDailyDataList.get(i).getAverageIncome();
            }
        }
        return retVal;
    }
